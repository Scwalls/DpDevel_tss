...
setwd("/scratch/scwalls/STRIPEseq_pipelines/GoSTRIPES_sing/STRIPES/tsr_Bs") 
...
names(Dp.tss) <- c("DpB_1", "DpB_2", "DpB_3")

Dp.tss.short <- list(tss.1.short.gr, tss.2.short.gr, tss.3.short.gr)
names(Dp.tss.short) <- c("DpB_1","DpB_2","DpB_3")
...
names(Dp.tsr) <- c("DpB_1", "DpB_2", "DpB_3")
...
ggsave(file="DpB_correlation_matrix.png") #saving the plot

### plot genomic distribution of TSSs

plot_genomic_distribution(exp, data_type="tss", samples=c("DpB_1","DpB_2","DpB_3")) +
  scale_fill_viridis_d(direction=-1, name="Annotation") 

ggsave(file="DpB_genomic_distribution.png") #saving the plot

### plot density

plot_density(exp, data_type="tss", samples=c("DpB_1", "DpB_2", "DpB_3"))
...
ggsave(file="DpB_density.png") #saving the plot

### plot TSS heatmap

plot_heatmap(
  exp, data_type="tss", samples=c("DpB_1","DpB_2","DpB_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggsave(file="DpB_tss_heatmap.png") #saving the plot
...
plot_heatmap(
  exp, data_type="tsr", samples=c("DpB_1","DpB_2","DpB_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggsave(file="DpB_tsr_heatmap.png") #saving the plot
...
plot_sequence_logo(exp_short, samples="DpB_1")
ggsave(file="DpulB_r1_sequenceLogo.png")
##### threshold exploration

plot_threshold_exploration(exp, samples="DpB_1", point_size=1) + scale_colour_viridis_c()
ggsave(file="DpulB_Count_Threshold.png")
exp <- apply_threshold(exp, threshold=3, n_samples=1)

plot_density(exp, data_type = "tss", samples = "DpB_1")
ggsave(file="DpulB_Densityplot.png")

plot_dinucleotide_frequencies(exp, samples = "DpB_1") + scale_fill_viridis_c()
