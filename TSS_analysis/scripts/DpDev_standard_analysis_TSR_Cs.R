...
setwd("/scratch/scwalls/STRIPEseq_pipelines/GoSTRIPES_sing/STRIPES/tsr_Cs") 
...
names(Dp.tss) <- c("DpC_1", "DpC_2", "DpC_3")

Dp.tss.short <- list(tss.1.short.gr, tss.2.short.gr, tss.3.short.gr)
names(Dp.tss.short) <- c("DpC_1","DpC_2","DpC_3")
...
names(Dp.tsr) <- c("DpC_1", "DpC_2", "DpC_3")
...
ggsave(file="DpC_correlation_matrix.png") #saving the plot

### plot genomic distribution of TSSs

plot_genomic_distribution(exp, data_type="tss", samples=c("DpC_1","DpC_2","DpC_3")) +
  scale_fill_viridis_d(direction=-1, name="Annotation") 

ggsave(file="DpC_genomic_distribution.png") #saving the plot

### plot density

plot_density(exp, data_type="tss", samples=c("DpC_1", "DpC_2", "DpC_3"))
...
ggsave(file="DpC_density.png") #saving the plot

### plot TSS heatmap

plot_heatmap(
  exp, data_type="tss", samples=c("DpC_1","DpC_2","DpC_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggsave(file="DpC_tss_heatmap.png") #saving the plot
...
plot_heatmap(
  exp, data_type="tsr", samples=c("DpC_1","DpC_2","DpC_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggsave(file="DpC_tsr_heatmap.png") #saving the plot
...
plot_sequence_logo(exp_short, samples="DpC_1")
ggsave(file="DpulC_r1_sequenceLogo.png")
##### threshold exploration

plot_threshold_exploration(exp, samples="DpC_1", point_size=1) + scale_colour_viridis_c()
ggsave(file="DpulC_Count_Threshold.png")
exp <- apply_threshold(exp, threshold=3, n_samples=1)

plot_density(exp, data_type = "tss", samples = "DpC_1")
ggsave(file="DpulC_Densityplot.png")

plot_dinucleotide_frequencies(exp, samples = "DpC_1") + scale_fill_viridis_c()
